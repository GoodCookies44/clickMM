...
// Функция для обновления счетчика, зависящего от суммы других счетчиков
const useSumUpdate = (targetId, sourceIds) => {
  const {counters, updateCounterValue} = useContext(CounterContext);

  useEffect(() => {
    const targetCounter = counters.find((counter) => counter.id === targetId);
    if (!targetCounter) return;

    const newValue = sourceIds.reduce((accumulator, sourceId) => {
      const sourceCounter = counters.find((counter) => counter.id === sourceId);
      if (sourceCounter) {
        accumulator += sourceCounter.value;
      }
      return accumulator;
    }, 0);

    updateCounterValue(targetId, newValue);
  }, [counters, targetId, sourceIds, updateCounterValue]);
};
...
export default function Counter({id}) {
  ...
  useSumUpdate("MFP_SHK", ["MFP_accepted", "MFP_rejected", "MFP_revision"]);
  useSumUpdate("DFP_request", ["DFP_accepted", "DFP_rejected"]);
  ...
  return (
    <div className="counter__container">
      ...
      <div className="button__container">
        <div onClick={increment} className="counter__button">
          {/* Иконка Плюс */}
          <svg
        ...
        <div onClick={decrement} className="counter__button">
          {/* Иконка Минус */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
        ...
        <div onClick={reset} className="counter__button">
          {/* Иконка Сброс */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
